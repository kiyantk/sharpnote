...
const ExportPopup = ({
  ...
  const [includeFoldersInExport, setIncludeFoldersInExport] = useState(false);
  ...
  const handleCheckboxChangeIncludeFolders = () => {
    setIncludeFoldersInExport((prev) => !prev);
  };
  ...
  const handleExport = async () => {
    ...
    const formatNoteForExport = (note) => ({
      ...note,
      noteFolder:
        includeFoldersInExport && exportType !== "single"
          ? note.noteFolder
          : null,
      noteHistory: {
    ...
    const getFoldersForNotes = () => {
      const folderSet = new Set();
      const folders = [];

      selectedNotes.forEach((note) => {
        const folder = allFolders.find((f) => f.folderID === note.noteFolder);
        if (folder && !folderSet.has(folder.folderID)) {
          folderSet.add(folder.folderID);
          folder.folderNotes = Array.isArray(folder.folderNotes)
            ? folder.folderNotes
    ...
    const getCurrentStructure = () => {
      if (!currSet.structure) return null;

      const { rootOrder, folders } = currSet.structure;
      const noteIDs = new Set(selectedNotes.map((note) => note.noteID));
      const folderIDs = new Set(
        includeFoldersInExport
          ? getFoldersForNotes().map((folder) => folder.folderID)
          : []
      );

      // Filter rootOrder to include only relevant noteIDs and folderIDs
      const filteredRootOrder = rootOrder.filter(
        (id) => noteIDs.has(id) || folderIDs.has(id)
      );

      // Filter folders to include only relevant folderIDs and their valid children
      const filteredFolders = folders
        .filter((folder) => folderIDs.has(folder.folderID))
        .map((folder) => ({
          folderID: folder.folderID,
          folderOrder: folder.folderOrder.filter(
            (id) => noteIDs.has(id) || folderIDs.has(id)
          ),
        }));

      return { rootOrder: filteredRootOrder, folders: filteredFolders };
    };
    ...
    if (exportType === "single" && selectedNotes?.length === 1) {
      ...
      if (exportFormat === "pack") {
        ...
        const bookContent = {
          ...
          bookAuthor: currSet.username,
          bookType: exportType,
          notes: selectedNotes.map(formatNoteForExport),
          folders: includeFoldersInExport ? getFoldersForNotes() : null,
          structure: getCurrentStructure(),
        };
        const blob = new Blob([JSON.stringify(bookContent, null, 2)], {
    ...
    <div className="settings-popup-overlay">
      <div className="noteinfo-popup">
        ...
        <div className="note-export-popup-content">
          <div className="export-popup-sidebyside">
            <div
              className="export-popup-item"
              style={{ justifyContent: "left" }}
            >
              <span>Export Type</span>
              <div className="export-popup-type-container">
                <div className="export-popup-type-icon">
            ...
            {(exportType === "all" ||
              ...
              >
                ...
                >
                  {exportType === "single" && (
                    <option value="single-sharp">
                      SharpNote Note File (*.sharp)
                    </option>
                  )}
                  {exportType === "single" && (
                    <option value="single-txt">TXT (*.txt)</option>
                  )}
                  {exportType === "single" && (
                    <option value="single-pdf">PDF (*.pdf)</option>
                  )}
                  {exportType === "single" && (
                    <option value="single-docx">Word Document (*.docx)</option>
                  )}
                  {exportType === "single" && (
                    <option value="single-html">
                      Single File Web Page (*.html)
                    </option>
                  )}
                  {(exportType === "all" || exportType === "selection") && (
                    <option value="pack">
                      Pack into a single file (*.sharpbook)
                    </option>
                  )}
                  {(exportType === "all" || exportType === "selection") && (
                  ...
                  {exportType === "database" && (
                    <option value="nopack">SQLite Database (*.db)</option>
                  )}
                </select>
              </div>
            )}
          </div>
          {(exportType === "single" || exportType === "selection") && (
            <div className="note-browser-container">
              <div
                className="note-browser-small"
                style={{
                  display:
                    exportType === "single" || exportType === "selection"
                      ? "block"
              ...
              >
                ...
                {allNotesCopy.map((note) => (
                  <div
                    key={note.noteID}
                    className={`note-browser-item hide-vertical-overflow-text ${
                      selectedNotes?.includes(note)
                        ? "note-browser-selected"
                        : ""
                    }`}
                    onClick={() =>
                      exportType === "single"
                  ...
                  >
                    <FontAwesomeIcon
                      ...
                      icon={
                        selectedNotes?.includes(note) ? faCircleCheck : faCircle
                      }
                    />
                    <FontAwesomeIcon
                      style={{ color: note.noteColor, marginRight: "5px" }}
                      icon={faStickyNote}
                    />
                    {note.noteTitle}
                  </div>
          ...
          {(exportType === "all" || exportType === "selection") &&
            exportFormat === "pack" && (
              <div className="export-popup-item export-popup-item-check">
                <input
                  type="checkbox"
                  checked={includeFoldersInExport}
                  onChange={handleCheckboxChangeIncludeFolders}
                />
                <span>Include folders in export</span>
              </div>
            )}
          {exportFormat !== "nopack" && (
